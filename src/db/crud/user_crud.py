...
def getUserId(user: UserData, DB: PostgresDB) -> None:
    id = DB.get_data(
        TABLE_NAME, ["u_id"], condition=("email", user.email), fetchall=False
    )
    return id


def updateUser(user_update: UserUpdate, DB: PostgresDB) -> Dict[str, str]:
    existing_user = DB.get_data(
        TABLE_NAME,
    ...
    if user_update.new_email:
        ...
        existing_email = DB.get_data(
            TABLE_NAME, ["email"], condition=("email", user_update.new_email)
        )
        if existing_email:
            raise HTTPException(status_code=400, detail="Email already in use")
    ...
    try:
        DB.update_data(
            TABLE_NAME, update_data, condition=("username", user_update.username)
        )
...
def deleteUser(user: UserData, DB: PostgresDB) -> Dict[str, str]:
    existing_user = DB.get_data(
        TABLE_NAME, ["username", "password", "email"], condition=("email", user.email)
    )
